...
def convolve_spectrum(wavs, flux, R, verbose=False):
    """
    ...
    # Calculate the width of the filter in terms of wavelength
    delta_lambda = wavs[int(len(wavs)/2)] / R

    # Calculate the width of the filter in terms of pixels
    FWHM = delta_lambda / np.mean(np.diff(wavs))

    # Calculate the std dev. of the gaussian in terms of pixels
